...
#pragma endregion
	...
	inline void Input::pollEventsAndUpdateTables() {
		...
		for (int i = 0; i < SDL_SCANCODE_COUNT; ++i) {
			if (m_keyTable[i] == FIRST_PRESSED) m_keyTable[i] = IS_PRESSED;
			else if (m_keyTable[i] == RELEASED_THIS_FRAME) m_keyTable[i] = NOT_PRESSED;
		}
		for (int i = 0; i < 5; ++i) {
			if (m_mouseKeysTable[i] == FIRST_PRESSED) m_mouseKeysTable[i] = IS_PRESSED;
			else if (m_mouseKeysTable[i] == RELEASED_THIS_FRAME) m_mouseKeysTable[i] = NOT_PRESSED;
		}
		...
		/*if (!SDL_GetKeyboardState(keyboardState))
		{
			return;
		}*/

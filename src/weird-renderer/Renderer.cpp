...
namespace WeirdEngine {
	namespace WeirdRenderer {
		...
		void Renderer::output(Scene& scene, Texture& texture)
		{
			...
			static int amplitudeMode = 6; // Change this value to test different modes (0 through 5)

			if (Input::GetKeyDown(Input::Z)) {
				amplitudeMode++;
				std::cout << "amplitudeMode: " << amplitudeMode << std::endl;
			}

			// --- Define parameters for the curves ---
			// These can be tweaked to get the desired feel.
			// You might want to make these class members or constants.
			const float MAX_FRICTION = 1.0f; // The expected maximum value from scene.getFrictionSound()
			const float MIN_AUDIBLE = 0.05f; // The minimum amplitude you want to hear
			const float MAX_AMPLITUDE = 0.8f; // The loudest you want the friction to be

			switch (amplitudeMode) {
				case 0: {
					// Simple Linear (your original code, for comparison)
					m_audioEngine.setFrictionLevel(std::min(0.5f * frictionValue, 0.1f));
					break;
				}
				...
				case 1: {
					...
					float normalizedFriction = std::min(frictionValue / MAX_FRICTION, 1.0f);
					float amplitude = MIN_AUDIBLE + (MAX_AMPLITUDE - MIN_AUDIBLE) * pow(normalizedFriction, 2.0f);
					// The '2.0f' makes it quadratic
					m_audioEngine.setFrictionLevel(amplitude);
					break;
				}
				...
				case 3: {
					...
					if (normalizedFriction > 0) {
						...
						m_audioEngine.setFrictionLevel(0.0f);
					}
					break;
				}
				...
				case 4: {
					...
					const float minFrictionLog = 0.01f; // Use a small epsilon to avoid log(0)
					if (frictionValue > minFrictionLog) {
						float normalizedFriction = std::min(frictionValue / MAX_FRICTION, 1.0f);
						// Map the friction value from a linear to a logarithmic scale
						...
						if (range > 0) {
							...
							m_audioEngine.setFrictionLevel(MIN_AUDIBLE);
						}
					} else {
						m_audioEngine.setFrictionLevel(0.0f);
					}
				...
				case 5: {
					...
					if (initialAmplitude > threshold) {
						...
						m_audioEngine.setFrictionLevel(finalAmplitude);
					} else {
						m_audioEngine.setFrictionLevel(initialAmplitude);
					}
					break;
				}
				case 6: { // Enhanced Compression (Stronger Low-End)
					// The compression settings are unchanged, as the top-end was perfect.

...
namespace WeirdEngine
{
	...
	namespace PhysicsInteractionSystem
	{
		...
		inline vec2 getMousePositionInWorld(Registry& registry);
		inline void drag(Registry& registry);
		inline void impulse(Registry& registry);
		inline void fix(Registry& registry);
		inline void spring(Registry& registry);
		inline void positionConstraint(Registry& registry);
		...
		inline void update(Registry& registry)
		{
			...
			if (getLeftClickDown())
			{
				...
				vec2 mousePositionInWorld = getMousePositionInWorld(registry);
			...
			{
				auto rbs = registry.getComponentArray<RigidBody2D>();

				RigidBody2D& target = rbs->getLastData();
				Entity targetOwner = rbs->getEntityAtIdx(rbs->getSize() - 1);

				vec3 position = registry.getComponent<Transform>(targetOwner).position;
		...
		inline vec2 getMousePositionInWorld(Registry& registry)
		{
			...
			auto& cameraTransform = registry.getComponent<Transform>(0); // m_mainCamera
		...
		inline void drag(Registry& registry)
		{
			if (getRightClickDown())
			{
				...
				vec2 mouseInWorld = getMousePositionInWorld(registry);

				for (int i = 0; i < rbs->getSize(); i++)
				{
					auto& rb = rbs->getDataAtIdx(i);
					Entity rbOwner = rbs->getEntityAtIdx(i);
					auto& t = registry.getComponent<Transform>(rbOwner);
				...
				{
					auto& rb = registry.getComponent<RigidBody2D>(m_dragId);
					rb.isFixed = true;
			...
			if (getRightClickUp())
			{
				if (m_dragId != INVALID_ENTITY)
				{
					...
					registry.getComponentArray<RigidBody2D>()->setEntityDirty(m_dragId, true);
			...
			{
				vec2 mousePositionInWorld = getMousePositionInWorld(registry);
				auto& t = registry.getComponent<Transform>(m_dragId);

				t.position = vec3(mousePositionInWorld, 0.0f);
				registry.getComponentArray<Transform>()->setEntityDirty(m_dragId, true);
			}
		}

		inline Entity getEntityAtPosition(Registry& registry, vec2 position, float radius = 0.5f)
		{
			auto rbs = registry.getComponentArray<RigidBody2D>();
		...
		inline void impulse(Registry& registry)
		{
			if (getRightClickDown())
			{
				...
				m_loadStartPosition = getMousePositionInWorld(registry);
			}
			...
			if (getRightClickUp())
			{
				...
				auto m_rbManager = registry.getComponentManager<RigidBody2D>();
		...
		inline void fix(Registry& registry)
		{
			if (getRightClickDown())
			{
				Entity id = getEntityAtPosition(registry, getMousePositionInWorld(registry));
				if (id != INVALID_ENTITY)
				{
					auto& rb = registry.getComponent<RigidBody2D>(id);
					rb.isFixed = !rb.isFixed;
					registry.getComponentArray<RigidBody2D>()->setEntityDirty(id, true);
				}
			}
		}

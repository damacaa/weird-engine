...
namespace WeirdRenderer
{
	...
	void Camera::Inputs(float delta)
	{
		...
		{
			m_locked = false;
			// Unhides cursor since m_camera is not looking around anymore
			Input::ShowMouse();
			// Makes sure the next time the m_camera looks around it doesn't jump
			firstClick = true;
		}
		...
		if (m_locked) {
			...
			// Prevents m_camera from jumping on the first click
			if (firstClick)
			{

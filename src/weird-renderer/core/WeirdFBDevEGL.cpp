...
namespace WeirdEngine
{
	namespace WeirdRenderer
	{
		...
		bool InitFBDevEGL(int& width, int& height)
		{
			...
			width = vinfo.xres;
			height = vinfo.yres;
			std::cout << "[FBDevEGL] fb0: " << width << "x" << height << " (virtual " << vinfo.xres_virtual << "x"
					  << vinfo.yres_virtual << ", " << vinfo.bits_per_pixel << " bpp)" << std::endl;
...
namespace WeirdEngine
{
	namespace WeirdRenderer
	{
		...
		bool InitFBDevEGL(int&, int&)
		{
			return false;
		}
		void ShutdownFBDevEGL() {}
		void SwapFBDevBuffers() {}
		void* GetEGLProcAddress(const char*)
		{
			return nullptr;
		}

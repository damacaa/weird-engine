...
const float u_weight[5] = float[5](0.227027, 0.1945946, 0.1216216, 0.054054, 0.016216);
uniform float u_time;

void main()
{
	vec2 tex_offset = 1.0 / vec2(textureSize(t_colorTexture, 0));
	...
	for (int i = 1; i < 5; ++i)
	{
		vec2 offset = u_horizontal
						  ...
						  : vec2(0.0, (tex_offset.y * float(i))); // (tex_offset.x * i) + hash(gl_FragCoord.xy + u_time)

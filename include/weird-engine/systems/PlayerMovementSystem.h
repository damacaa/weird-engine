					...
					[&](Entity target, FlyMovement2D& flyComponent, Transform& t, Camera& c)
					{
						...
						if (m_locked2D)
						{
							...
							else
							{
								...
								targetPosition +=
									flyComponent.speed * vec3(-Input::GetMouseDeltaXRaw() * worldUnitsPerPixel,
						...
						if (Input::GetMouseButton(Input::WheelDown))
						{
							...
							vec2 cursorPosition = Camera::screenPositionToWorldPosition2D(
								t, vec2(Input::GetMouseX(), Input::GetMouseY()));
						...
						{
							// Exponential smoothing is frame-rate independent and resilient to occasional large frame
							// times.

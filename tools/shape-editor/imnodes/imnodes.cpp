...
namespace IMNODES_NAMESPACE
{
	namespace
	{
		...
		inline ImVec2 EvalCubicBezier(const float t, const ImVec2& P0, const ImVec2& P1, const ImVec2& P2,
									  const ImVec2& P3)
		{
			...
			return ImVec2(b0 * P0.x + b1 * P1.x + b2 * P2.x + b3 * P3.x, b0 * P0.y + b1 * P1.y + b2 * P2.y + b3 * P3.y);
		}
		...
		inline float GetDistanceToCubicBezier(const ImVec2& pos, const CubicBezier& cubic_bezier,
		...
		inline CubicBezier GetCubicBezier(ImVec2 start, ImVec2 end, const ImNodesAttributeType start_type,
		...
		inline bool RectangleOverlapsLineSegment(const ImRect& rect, const ImVec2& p1, const ImVec2& p2)
		{
			...
			const int corner_signs[4] = {Sign(EvalImplicitLineEq(p1, p2, flip_rect.Min)),
		...
		{
			ImVec2 current = EvalCubicBezier(0.f, cubic_bezier.P0, cubic_bezier.P1, cubic_bezier.P2, cubic_bezier.P3);
			const float dt = 1.0f / cubic_bezier.NumSegments;
			for (int s = 0; s < cubic_bezier.NumSegments; ++s)
			{
				ImVec2 next = EvalCubicBezier(static_cast<float>((s + 1) * dt), cubic_bezier.P0, cubic_bezier.P1,
		...
		inline bool RectangleOverlapsLink(const ImRect& rectangle, const ImVec2& start, const ImVec2& end,
		...
		{
			return (ScreenSpaceToGridSpace(editor, v) - editor.GridContentBounds.Min) * editor.MiniMapScaling +
		...
		{
			return ImRect(ScreenSpaceToMiniMapSpace(editor, r.Min), ScreenSpaceToMiniMapSpace(editor, r.Max));
		}
		...
		void ImDrawListSplitterSwapChannels(ImDrawListSplitter& splitter, const int lhs_idx, const int rhs_idx)
		{
			if (lhs_idx == rhs_idx)
			{
				return;
		...
		{
			GImNodes->NodeIdxToSubmissionIdx.SetInt(static_cast<ImGuiID>(node_idx),
		...
		{
			GImNodes->CanvasDrawList->_Splitter.SetCurrentChannel(GImNodes->CanvasDrawList,
		...
		void DrawListActivateCurrentNodeForeground()
		{
			...
			GImNodes->CanvasDrawList->_Splitter.SetCurrentChannel(GImNodes->CanvasDrawList, foreground_channel_idx);
		}
		...
		void DrawListSwapSubmissionIndices(const int lhs_idx, const int rhs_idx)
		{
			...
			ImDrawListSplitterSwapChannels(GImNodes->CanvasDrawList->_Splitter, lhs_background_channel_idx,
										   rhs_background_channel_idx);
			ImDrawListSplitterSwapChannels(GImNodes->CanvasDrawList->_Splitter, lhs_foreground_channel_idx,
		...
		ImVec2 GetScreenSpacePinCoordinates(const ImRect& node_rect, const ImRect& attribute_rect,
											const ImNodesAttributeType type, const float zoom = 1.0f)
		{
			IM_ASSERT(type == ImNodesAttributeType_Input || type == ImNodesAttributeType_Output);
			const float pin_offset = GImNodes->Style.PinOffset * zoom;
			const float x =
				type == ImNodesAttributeType_Input ? (node_rect.Min.x - pin_offset) : (node_rect.Max.x + pin_offset);
		...
		ImVec2 GetScreenSpacePinCoordinates(const ImNodesEditorContext& editor, const ImPinData& pin)
		{
			...
			return GetScreenSpacePinCoordinates(parent_node_rect, pin.AttributeRect, pin.Type, editor.Zoom);
		}
		...
		void BeginLinkDetach(ImNodesEditorContext& editor, const int link_idx, const int detach_pin_idx)
		{
			...
			state.LinkCreation.StartPinIdx = detach_pin_idx == link.StartPinIdx ? link.EndPinIdx : link.StartPinIdx;
		...
		void TranslateSelectedNodes(ImNodesEditorContext& editor)
		{
			if (GImNodes->LeftMouseDragging)
			{
				...
				const ImVec2 origin =
					SnapOriginToGrid(ScreenSpaceToGridSpace(editor, GImNodes->MousePos + editor.PrimaryNodeOffset));
		...
		ImOptionalIndex FindDuplicateLink(const ImNodesEditorContext& editor, const int start_pin_idx,
		...
		bool ShouldLinkSnapToPin(const ImNodesEditorContext& editor, const ImPinData& start_pin,
								 const int hovered_pin_idx, const ImOptionalIndex duplicate_link)
		{
		...
		void ClickInteractionUpdate(ImNodesEditorContext& editor)
		{
			switch (editor.ClickInteraction.Type)
			{
				...
				{
					const ImPinData& start_pin = editor.Pins.Pool[editor.ClickInteraction.LinkCreation.StartPinIdx];

					const ImOptionalIndex maybe_duplicate_link_idx =
						GImNodes->HoveredPinIdx.HasValue()
							? FindDuplicateLink(editor, editor.ClickInteraction.LinkCreation.StartPinIdx,
					...
					const ImVec2 end_pos =
						should_snap
							? GetScreenSpacePinCoordinates(editor, editor.Pins.Pool[GImNodes->HoveredPinIdx.Value()])
							: GImNodes->MousePos;

					const CubicBezier cubic_bezier =
						GetCubicBezier(start_pos, end_pos, start_pin.Type, GImNodes->Style.LinkLineSegmentsPerLength);
					...
					GImNodes->CanvasDrawList->AddBezierCubic(
						...
						ImMax(1.0f, GImNodes->Style.LinkThickness * editor.Zoom), cubic_bezier.NumSegments);

					const bool link_creation_on_snap =
						GImNodes->HoveredPinIdx.HasValue() && (editor.Pins.Pool[GImNodes->HoveredPinIdx.Value()].Flags &
					...
					const bool create_link = should_snap && (GImNodes->LeftMouseReleased || link_creation_on_snap);
		...
		ImOptionalIndex ResolveHoveredPin(const ImNodesEditorContext& editor, const ImObjectPool<ImPinData>& pins,
										  const ImVector<int>& occluded_pin_indices)
		{
			float smallest_distance = FLT_MAX;
			ImOptionalIndex pin_idx_with_smallest_distance;

			const float hover_radius = GImNodes->Style.PinHoverRadius * editor.Zoom;
		...
		ImOptionalIndex ResolveHoveredLink(const ImNodesEditorContext& editor, const ImObjectPool<ImLinkData>& links,
										   const ImObjectPool<ImPinData>& pins)
		{
			...
			for (int idx = 0; idx < links.Pool.Size; ++idx)
			{
				...
				const CubicBezier cubic_bezier = GetCubicBezier(start_pin.Pos, end_pin.Pos, start_pin.Type,
				...
				// The distance test
				{
					...
					if (link_rect.Contains(GImNodes->MousePos))
					{
						...
						const float hover_distance = GImNodes->Style.LinkHoverDistance * editor.Zoom;
						if (distance < hover_distance && distance < smallest_distance)
						{
		...
		inline ImVec2 GetNodeTitleBarOrigin(const ImNodesEditorContext& editor, const ImNodeData& node)
		{
			return GridSpaceToEditorSpace(editor, node.Origin) + node.LayoutStyle.Padding;
		}

		inline ImVec2 GetNodeContentOrigin(const ImNodesEditorContext& editor, const ImNodeData& node)
		{
			const ImVec2 title_bar_height =
				ImVec2(0.f, node.TitleBarContentRect.GetHeight() + 2.0f * node.LayoutStyle.Padding.y);
			return GridSpaceToEditorSpace(editor, node.Origin) + title_bar_height + node.LayoutStyle.Padding;
		}
		...
		inline ImRect GetNodeTitleRect(const ImNodeData& node)
		{
			...
			return ImRect(expanded_title_rect.Min, expanded_title_rect.Min + ImVec2(node.Rect.GetWidth(), 0.f) +
		...
		void DrawGrid(ImNodesEditorContext& editor, const ImVec2& canvas_size)
		{
			...
			float spacing = GImNodes->Style.GridSpacing * editor.Zoom;
			while (spacing < 16.0f)
				spacing *= 2.0f;
			...
			for (float x = fmodf(offset.x, spacing); x < canvas_size.x; x += spacing)
			{
				GImNodes->CanvasDrawList->AddLine(EditorSpaceToScreenSpace(ImVec2(x, 0.0f)),
												  EditorSpaceToScreenSpace(ImVec2(x, canvas_size.y)), line_color);
			}
			...
			for (float y = fmodf(offset.y, spacing); y < canvas_size.y; y += spacing)
			{
				GImNodes->CanvasDrawList->AddLine(EditorSpaceToScreenSpace(ImVec2(0.0f, y)),
												  EditorSpaceToScreenSpace(ImVec2(canvas_size.x, y)), line_color);
			}
		}
		...
		void DrawPinShape(const ImVec2& pin_pos, const ImPinData& pin, const ImU32 pin_color, const float zoom = 1.0f)
		{
			...
			switch (pin.Shape)
			{
				...
				{
					GImNodes->CanvasDrawList->AddCircle(pin_pos, GImNodes->Style.PinCircleRadius * zoom, pin_color,
														CIRCLE_NUM_SEGMENTS,
														ImMax(1.0f, GImNodes->Style.PinLineThickness * zoom));
				}
				break;
				case ImNodesPinShape_CircleFilled:
				{
					GImNodes->CanvasDrawList->AddCircleFilled(pin_pos, GImNodes->Style.PinCircleRadius * zoom,
															  pin_color, CIRCLE_NUM_SEGMENTS);
				}
				break;
				case ImNodesPinShape_Quad:
				{
					const QuadOffsets offset = CalculateQuadOffsets(GImNodes->Style.PinQuadSideLength * zoom);
					GImNodes->CanvasDrawList->AddQuad(pin_pos + offset.TopLeft, pin_pos + offset.BottomLeft,
													  pin_pos + offset.BottomRight, pin_pos + offset.TopRight,
													  pin_color, ImMax(1.0f, GImNodes->Style.PinLineThickness * zoom));
				}
				break;
				case ImNodesPinShape_QuadFilled:
				{
					const QuadOffsets offset = CalculateQuadOffsets(GImNodes->Style.PinQuadSideLength * zoom);
					GImNodes->CanvasDrawList->AddQuadFilled(pin_pos + offset.TopLeft, pin_pos + offset.BottomLeft,
															pin_pos + offset.BottomRight, pin_pos + offset.TopRight,
															pin_color);
				}
				break;
				case ImNodesPinShape_Triangle:
				{
					const TriangleOffsets offset =
						CalculateTriangleOffsets(GImNodes->Style.PinTriangleSideLength * zoom);
					GImNodes->CanvasDrawList->AddTriangle(pin_pos + offset.TopLeft, pin_pos + offset.BottomLeft,
														  pin_pos + offset.Right, pin_color,
														  // NOTE: for some weird reason, the line drawn by AddTriangle
														  // is much thinner than the lines drawn by AddCircle or
														  // AddQuad. Multiplying the line thickness by two seemed to
														  // solve the problem at a few different thickness values.
														  ImMax(1.0f, 2.f * GImNodes->Style.PinLineThickness * zoom));
				}
				break;
				case ImNodesPinShape_TriangleFilled:
				{
					const TriangleOffsets offset =
						CalculateTriangleOffsets(GImNodes->Style.PinTriangleSideLength * zoom);
					GImNodes->CanvasDrawList->AddTriangleFilled(pin_pos + offset.TopLeft, pin_pos + offset.BottomLeft,
																pin_pos + offset.Right, pin_color);
				}
				break;
				default:
		...
		void DrawPin(ImNodesEditorContext& editor, const int pin_idx)
		{
			...
			pin.Pos = GetScreenSpacePinCoordinates(parent_node_rect, pin.AttributeRect, pin.Type, editor.Zoom);
			...
			DrawPinShape(pin.Pos, pin, pin_color, editor.Zoom);
		}
		...
		void DrawNode(ImNodesEditorContext& editor, const int node_idx)
		{
			...
			{
				// node base
				GImNodes->CanvasDrawList->AddRectFilled(node.Rect.Min, node.Rect.Max, node_background,
				...
				if (node.TitleBarContentRect.GetHeight() > 0.f)
				{
					...
					GImNodes->CanvasDrawList->AddRectFilled(title_bar_rect.Min, title_bar_rect.Max, titlebar_background,
				...
				{
#if IMGUI_VERSION_NUM < 18200
					GImNodes->CanvasDrawList->AddRect(node.Rect.Min, node.Rect.Max, node.ColorStyle.Outline,
...
#else
					GImNodes->CanvasDrawList->AddRect(node.Rect.Min, node.Rect.Max, node.ColorStyle.Outline,
													  node.LayoutStyle.CornerRounding, node.LayoutStyle.BorderThickness,
		...
		void DrawLink(ImNodesEditorContext& editor, const int link_idx)
		{
			...
			const CubicBezier cubic_bezier =
				GetCubicBezier(start_pin.Pos, end_pin.Pos, start_pin.Type, GImNodes->Style.LinkLineSegmentsPerLength);
			...
			GImNodes->CanvasDrawList->AddBezierCubic(
#endif
				cubic_bezier.P0, cubic_bezier.P1, cubic_bezier.P2, cubic_bezier.P3, link_color,
				ImMax(1.0f, GImNodes->Style.LinkThickness * editor.Zoom), cubic_bezier.NumSegments);
		}

		void BeginPinAttribute(const int id, const ImNodesAttributeType type, const ImNodesPinShape shape,
							   const int node_idx)
		{
		...
		static inline void CalcMiniMapLayout()
		{
			...
			float mini_map_scaling;
			{
				...
				const ImVec2 grid_content_size =
					editor.GridContentBounds.IsInverted() ? max_size : ImFloor(editor.GridContentBounds.GetSize());
		...
		static void MiniMapDrawNode(ImNodesEditorContext& editor, const int node_idx)
		{
			...
			const float mini_map_node_rounding = floorf(node.LayoutStyle.CornerRounding * editor.MiniMapScaling);
			...
			GImNodes->CanvasDrawList->AddRectFilled(node_rect.Min, node_rect.Max, mini_map_node_background,
													mini_map_node_rounding);

			GImNodes->CanvasDrawList->AddRect(node_rect.Min, node_rect.Max, mini_map_node_outline,
											  mini_map_node_rounding);
		}
		...
		static void MiniMapDrawLink(ImNodesEditorContext& editor, const int link_idx)
		{
			...
			const CubicBezier cubic_bezier = GetCubicBezier(
				ScreenSpaceToMiniMapSpace(editor, start_pin.Pos), ScreenSpaceToMiniMapSpace(editor, end_pin.Pos),
				start_pin.Type, GImNodes->Style.LinkLineSegmentsPerLength / editor.MiniMapScaling);
			...
			const ImU32 link_color =
				GImNodes->Style.Colors[editor.SelectedLinkIndices.contains(link_idx) ? ImNodesCol_MiniMapLinkSelected
...
#endif
				cubic_bezier.P0, cubic_bezier.P1, cubic_bezier.P2, cubic_bezier.P3, link_color,
				GImNodes->Style.LinkThickness * editor.MiniMapScaling, cubic_bezier.NumSegments);
		}
		...
		static void MiniMapUpdate()
		{
			...
			GImNodes->CanvasDrawList->AddRectFilled(mini_map_rect.Min, mini_map_rect.Max, mini_map_background);

			GImNodes->CanvasDrawList->AddRect(mini_map_rect.Min, mini_map_rect.Max,
											  GImNodes->Style.Colors[ImNodesCol_MiniMapOutline]);

			// Clip draw list items to mini-map rect (after drawing background/outline)
			GImNodes->CanvasDrawList->PushClipRect(mini_map_rect.Min, mini_map_rect.Max,
		...
		template <typename T>
		void ClearObjectSelection(const ImObjectPool<T>& objects, ImVector<int>& selected_indices, const int id)
		{
			const int idx = ObjectPoolFind(objects, id);
			IM_ASSERT(idx >= 0);
			IM_ASSERT(selected_indices.find(idx) != selected_indices.end());
...
namespace IMNODES_NAMESPACE
{
	...
	void EditorContextSetZoom(const float zoom, const ImVec2& center)
	{
		ImNodesEditorContext& editor = EditorContextGet();
		const float new_zoom = ImClamp(zoom, 0.25f, 3.0f);
		if (new_zoom == editor.Zoom)
		{
			return;
		}
		const ImVec2 grid = (center - GImNodes->CanvasOriginScreenSpace - editor.Panning) / editor.Zoom;
		editor.Zoom = new_zoom;
		editor.Panning = center - GImNodes->CanvasOriginScreenSpace - grid * editor.Zoom;
	}
	...
	void EditorContextResetZoom()
	{
		...
		EditorContextSetZoom(1.0f, center);
	}
	...
	void StyleColorsDark(ImNodesStyle* dest)
	{
		...
		dest->Colors[ImNodesCol_GridBackground] = IM_COL32(18, 22, 28, 255);
	...
	void BeginNodeEditor()
	{
		...
		GImNodes->MultipleSelectModifier =
			(GImNodes->Io.MultipleSelectModifier.Modifier != NULL ? *GImNodes->Io.MultipleSelectModifier.Modifier
																  : ImGui::GetIO().KeyCtrl);
		...
		ImGui::BeginGroup();
		{
			...
			ImGui::BeginChild("scrolling_region", ImVec2(0.f, 0.f), false,
...

			{
				...
				// Always draw completely opaque background quad to cover underlying scene shapes
				const ImVec2 canvas_min = GImNodes->CanvasOriginScreenSpace;
				const ImVec2 canvas_max = canvas_min + canvas_size;
				GImNodes->CanvasDrawList->AddRectFilled(canvas_min, canvas_max,
	...
	void EndNodeEditor()
	{
			...
			MouseInCanvas() && !IsMiniMapHovered())
		{
			...
			GImNodes->HoveredPinIdx = ResolveHoveredPin(editor, editor.Pins, GImNodes->OccludedPinIndices);
		...
		if (!IsMiniMapHovered())
		{
			...
			if (MouseInCanvas() && GImNodes->AltMouseScrollDelta != 0.0f && !ImGui::IsAnyItemActive() &&
				editor.ClickInteraction.Type == ImNodesClickInteractionType_None)
			{
				const float wheel = GImNodes->AltMouseScrollDelta;
				const float factor = (wheel > 0.0f) ? 1.12f : (1.0f / 1.12f);
				EditorContextSetZoom(editor.Zoom * factor, GImNodes->MousePos);
			}
			...
			else if (GImNodes->LeftMouseClicked || GImNodes->LeftMouseReleased || GImNodes->AltMouseClicked ||
					 GImNodes->AltMouseScrollDelta != 0.f)
			{
				BeginCanvasInteraction(editor);
			}

			bool should_auto_pan = editor.ClickInteraction.Type == ImNodesClickInteractionType_BoxSelection ||
								   editor.ClickInteraction.Type == ImNodesClickInteractionType_LinkCreation ||
								   editor.ClickInteraction.Type == ImNodesClickInteractionType_Node;
	...
	void MiniMap(const float minimap_size_fraction, const ImNodesMiniMapLocation location,
	...
	void BeginNode(const int node_id)
	{
		...
		node.LayoutStyle.CornerRounding = GImNodes->Style.NodeCornerRounding * editor.Zoom;
		node.LayoutStyle.Padding = GImNodes->Style.NodePadding * editor.Zoom;
		node.LayoutStyle.BorderThickness = GImNodes->Style.NodeBorderThickness * editor.Zoom;
		...
		ImGui::SetCursorPos(GetNodeTitleBarOrigin(editor, node));
	...
	void EndNodeTitleBar()
	{
		...
		ImGui::SetCursorPos(GetNodeContentOrigin(editor, node));
	}
	...
	bool IsLinkCreated(int* const started_at_pin_id, int* const ended_at_pin_id, bool* const created_from_snap)
	{
		IM_ASSERT(GImNodes->CurrentScope == ImNodesScope_None);
	...
	bool IsLinkCreated(int* started_at_node_id, int* started_at_pin_id, int* ended_at_node_id, int* ended_at_pin_id,
	...
	const char* SaveEditorStateToIniString(const ImNodesEditorContext* const editor_ptr, size_t* const data_size)
	{
	...
	void LoadEditorStateFromIniString(ImNodesEditorContext* const editor_ptr, const char* const data,
